...
Use [[#extra-shield][extra_shield]] for optional non-keyboard shields.


****** extra_shield

Specifies optional non-keyboard ZMK shields without changing which board or keyboard shield Miryoku uses to select its keymap and configuration.

For an onboard controller keyboard with a display shield, leave [[#shield][shield]] as ~default~ and enter the display shield here, e.g. ~nice_view~.

For a composite keyboard, enter the keyboard in [[#shield][shield]] and accessories here, e.g. use ~corne_left~ as the shield and ~nice_view_adapter nice_view~ as the extra shield. Separate multiple accessory shields with spaces. For multiple builds in the same run, separate extra-shield combinations with commas.
